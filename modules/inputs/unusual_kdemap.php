<?php
function getsetup_input_unusual_kdemap()
{
        $options['_CREDITS']                            = 'Main Developters: Robert Pahle, Jaycen Horton.';
        $options['_MODULEDESCRIPTION']          = 'This module displays kde map with unusual cell highlighted.';
...
function validate_unusual_kdemap($id, $value, $function=null)
{
    global $db;
...
function place_input_unusual_kdemap($sid, $value, $options, $setup)
{
    global $db;

    $data = reload_input_unusual_kdemap($sid, $value, $options, $setup);
    $extent  = $data['extent'];
...
function reload_input_unusual_kdemap($sid, $value, $options, $setup)
{
    global $db;
    ...
    {
        $sql = 'SELECT extent,imgpath FROM unusual_kde where '
...
function unusual_kdemap_rtrn_default_value($sid, $options)
{
}
