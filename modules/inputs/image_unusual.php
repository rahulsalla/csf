<?
function getsetup_input_image_unusual()
{
	...
	$options[30]['dependenton']			= '';
        ...
        $options[31]['name']                            = 'image2';
        $options[31]['description']                     = 'Please enter the href if the image';
        $options[31]['detail']                          = 'This sets the link to the image.';
        $options[31]['type']                            = 'Text';
        $options[31]['link']                            = 'link to further information..?';
        $options[31]['lookup']                          = ''; #this would be if i have a dropdown and want to lookup the value you would enter a sql script with the column that comes back being the selection^M
        $options[31]['default']                         = '';
        $options[31]['optional']                        = 'no';
        $options[31]['repeatable']                      = 'no';
        $options[31]['perdashboard']            = 'no';
...
function validate_image_unusual($id, $value)
{  
	global $db;
...
function place_input_image_unusual($sid, $value, $options, $setup)
{
	$dashboard_options = $options['dashboard_options'];

	echo '<div id="cover'.$sid.'">';
	echo reload_input_image_unusual($sid, $value, $options, $setup);
	echo '</div>';
...
function reload_input_image_unusual($sid, $value, $options, $setup)
{
	$dashboard_options = $options['dashboard_options'];

	if($value == 1) $render_image = $options['image'];
	else $render_image = $options['image2'];
	 ...
	 $content .= '<INPUT STYLE="'.($dashboard_options['css']).'" TYPE="image" src="'.$render_image.'" id="'.$setup['value_svid'].'" value="'.$value.'" onclick="validate_image(this.id, this.value, {\'onUpdate\': function(response,xmlhttp){';
...
function image_unusual_rtrn_default_value($sid, $options)
{
}
