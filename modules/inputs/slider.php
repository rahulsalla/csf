...
function getsetup_input_slider()
{
...
function validate_slider($id, $value)
{  
	global $db;
...
function place_input_slider($sid, $value, $options, $setup)
{	
	...
	$str .= '</div>';
	$str .= reload_input_slider($sid, $value, $options, $setup);
	$str .= '</div>';
	$str .= '</div>';
    ...
    function reload_input_slider($sid, $value, $options, $setup)
    {	
	$dashboard_options = $options['dashboard_options'];
...
//			top : '.$dashboard_options['setMaxTop'].'px;
	...
	//		left : '.$dashboard_options['setMaxLeft'].'px;
  		 ...
  		 //	background-color : '.$dashboard_options['sliderColor'].';
			...
			/*Slider bar thumb Style
 			...
 			input[type="range"].slider1'.$sid.'::-webkit-slider-thumb
			{
			-webkit-appearance : none;
	...
	//		-moz-box-shadow : 0px 6px 5px 0px rgba(0,0,0,0.6);
			...
			<input id="leftButton'.$sid.'" type="button"  onclick="moveLeft'.$sid.'(slider1'.$sid.')" onmouseout="myVar'.$sid.'(slider1'.$sid.')">
			<input type="range"  id="slider1'.$sid.'" STYLE="'.($dashboard_options['css']).'; 
	
			width:'.$dashboard_options['width'].'px;  "  min="'.$options['min'].'" ; max="'.($max).'" ;
			  step="'.$step.'" ;   id="'.$setup['value_svid'].'" ; value="'.$value.'" ; 
			  onmousemove="showValue'.$sid.'(this.value,this.min,this.max,this.step)"; class="slider1'.$sid.'";
			 onmouseup="myVar'.$sid.'(slider1'.$sid.')"/>';
			$content.='
			<input id="rightButton'.$sid.'" type="button" onclick="moveRight'.$sid.'(slider1'.$sid.')"  onmouseout="myVar'.$sid.'(slider1'.$sid.')"/>
			...
			if('.$options['min'].' <= '.$dashboard_options['setMinAllowed'].')
			var minAllowed'.$sid.' = '.$dashboard_options['setMinAllowed'].';	
			var flag = '.$flag.';
			...
			document.getElementById("max'.$sid.'").innerHTML = '.$max.';
			    ...
			    */		
				...
				{	
			 		validate_slider('.$setup['value_svid'].', slide.value, 
					{
				    	\'onUpdate\': function(response,xmlhttp)
				    	   {
						make_update(\''.$sid.'\',response);
				    	   }
					});
    ...
    function slider_rtrn_default_value($sid, $options)
    {
    }
