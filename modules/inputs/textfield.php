<?
function getsetup_input_textfield()
{
...
function validate_textfield($id, $value)
{  
...
function place_input_textfield($sid, $value, $options, $setup)	//function place_input_text($name, $value, $x, $y, $options, $sid)
{
	...
	echo '</div>';
	echo reload_input_textfield($sid, $value, $options, $setup);
	echo '</div>';
...
function reload_input_textfield($sid, $value, $options, $setup)	//function reload_input_text($name, $value, $x, $y, $options, $sid)
{
	global $db;
	...
	if(isset($options['tablename']) and ($options['tablename']!='') and isset($options['valuefield']) and ($options['valuefield']!='') and isset($options['queryfield']) and ($options['queryfield']!='') and isset($options['idnumber']) and ($options['idnumber']!='') )
	{
		...
		$content = '<div id="element'.$sid.'" style="z-index:2; position:absolute; top:'.($dashboard_options['y']).'; left:'.($dashboard_options['x']).'; width:150; height:10;">
			<INPUT CLASS=SmallInput STYLE="text-align: left; width:'.$dashboard_options['width'].'px;height:'.$dashboard_options['height'].'px; margin-left: 4px; margin-top: 4px;" TYPE=text SIZE=10 id="'.$setup['value_svid'].'" value="'.$value.'" onchange="validate_textfield(this.id, this.value, {\'onUpdate\': function(response,xmlhttp){make_update(\''.$sid.'\',response)}});">
			</div>';
	
	return($content);
}
function textfield_rtrn_default_value($sid, $options)
{
	global $db;
	if(isset($options['tablename']) and ($options['tablename']!='') and isset($options['valuefield']) and ($options['valuefield']!='') and isset($options['queryfield']) and ($options['queryfield']!='') and isset($options['idnumber']) and ($options['idnumber']!='') )
	{
