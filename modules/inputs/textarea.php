<?
function getsetup_input_textarea()
{
	$options['_CREDITS']				= 'Main Developters: Robert Pahle, Jaycen Horton, Shrijal Gandhi';
...
function validate_textarea($id, $value)
{  
...
function place_input_textarea($sid, $value, $options, $setup)	//function place_input_text($name, $value, $x, $y, $options, $sid)
{
	...
	echo '</div>';
	echo reload_input_textarea($sid, $value, $options, $setup);
	echo '</div>';
...
function reload_input_textarea($sid, $value, $options, $setup)	//function reload_input_text($name, $value, $x, $y, $options, $sid)
{
	global $db;
	//echo "<script type='text/javascript'>alert('$sid')</script>";	
	$dashboard_options = $options['dashboard_options'];
	...
	//		<INPUT CLASS=SmallInput STYLE="text-align: left; width:'.$dashboard_options['width'].'px;height:'.$dashboard_options['height'].'px; margin-left: 4px; margin-top: 4px;" TYPE=text SIZE=10 id="'.$setup['value_svid'].'" value="'.$value.'" onchange="validate_textarea(this.id, this.value, {\'onUpdate\': function(response,xmlhttp){make_update(\''.$sid.'\',response)}});">
	//		</div>';
//	
// you can Include in the dashboard options resize overflow row col etc properties
 $content = '<script language = "Javascript" type = "text/javascript">
		function adjust(id) 
		    { // this automatically resizes the height of the field
    			id.style.height = "1px";
   	 		id.style.height = (25+id.scrollHeight)+"px";
		    }
		</script>';

 $content .= '<div id="element'.$sid.'" style="z-index:2; position:absolute; top:'.($dashboard_options['y']).'; left:'.($dashboard_options['x']).'; width:150; height:10;">
 <textarea CLASS= SmallInput STYLE="text-align: left; width:'.$dashboard_options['width'].'px;height:'.$dashboard_options['height'].'px; margin-left: 4px; margin-top: 4px; overflow:hidden resize:both" TYPE=text SIZE=10 id="'.$setup['value_svid'].'" value="'.$value.'" onkeyup ="adjust(this)" onchange="validate_textarea(this.id, this.value, {\'onUpdate\': function(response,xmlhttp){make_update(\''.$sid.'\',response)}});">'.$value.'</textarea> </div>';//^M



	return($content);
}
function textarea_rtrn_default_value($sid, $options)
{
	global $db;
	if(isset($options['tablename']) and ($options['tablename']!='') and isset($options['valuefield']) and ($options['valuefield']!='') and isset($options['queryfield']) and ($options['queryfield']!='') and isset($options['idnumber']) and ($options['idnumber']!='') )
	{
