...
function getsetup_input_integer()
{
	...
	$options[30]['detail']				= 'This sets the width of the Integer field.';
...
function reload_input_integer($sid, $value, $options, $setup)
{
	$dashboard_options = $options['dashboard_options'];
	  $content = '<div id="element'.$sid.'" style="'.($dashboard_options['css']).'z-index:10; position:absolute; top:'.($dashboard_options['y']).'; left:'.($dashboard_options['x']).'; ">
			<INPUT CLASS=SmallInput STYLE="'.($dashboard_options['css']).'text-align: left;width:'.$dashboard_options['width'].'px;height:'.$dashboard_options['height'].'px; margin-left: 4px; margin-top: 4px; padding-left: 5px" TYPE=text  id="'.$setup['value_svid'].'" value="'.$value.'" onchange="validate_integer(this.id, this.value, {\'onUpdate\': function(response,xmlhttp){make_update(\''.$sid.'\',response)}});">
		  </div>';
	return($content);
}
