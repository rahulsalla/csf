...
function getsetup_viz_ImageAreaChart()
{
	...
	$options[120]['detail']				= 'displays charts verticall or horizontally. Default: Vertical';
	$options[120]['type']				= 'Dropdown';
	$options[120]['link']				= 'link to further information..?';
	$options[120]['lookup']				= '|Vertical|Horizontal'; 
	$options[120]['default']			= 'Vertical';
	...
	$options[140]['detail']				= 'Whether there will be fill under the lines of the chart. The options are true or false. Default: false';
...
function place_viz_ImageAreaChart($sid, $value, $options, $setup)
{
	...
	if(isset($options['tablename']))
	{
		...
		$content .= '
			...
			{';
		$content .= reload_viz_ImageAreaChart($sid, $value, $options, $setup);
		
		$content .=
		  'area_chart'.$sid.' = new google.visualization.ImageAreaChart(document.getElementById(\'velement'.$sid.'\')).draw(data'.$sid.', 
                 { 
...
function reload_viz_ImageAreaChart($sid, $value, $options, $setup)
{
	global $db;
