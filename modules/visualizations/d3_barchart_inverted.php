<?
function getsetup_viz_d3_barchart_inverted()
{
 ...
 For more information about D3 library, please, refer to      <a href="http:d3js.org">Data-Driven Documents</a>';
	...
	$options[40]['dependenton']			= '';	
        ...
        $options[41]['name']                            = 'minValue';
        $options[41]['description']                     = 'Minimum value of the chart y axis';
        $options[41]['detail']                          = 'Default: 500';
        $options[41]['type']                            = 'Integer';
        $options[41]['link']                            = 'link to further information..?';
        $options[41]['lookup']                          = '';
        $options[41]['default']                         = '500';
        $options[41]['optional']                        = 'no';
        $options[41]['repeatable']                      = 'no';
        $options[41]['perdashboard']                    = 'yes';
        $options[41]['dependenton']                     = '';
      
	$options[42]['name']                            = 'maxValue';
        $options[42]['description']                     = 'Maximum value of the chart y axis';
        $options[42]['detail']                          = 'Default: 500';
        $options[42]['type']                            = 'Integer';
        $options[42]['link']                            = 'link to further information..?';
        $options[42]['lookup']                          = '';
        $options[42]['default']                         = '500';
        $options[42]['optional']                        = 'no';
        $options[42]['repeatable']                      = 'no';
        $options[42]['perdashboard']                    = 'yes';
...
function place_viz_d3_barchart_inverted($sid, $value, $options, $setup)
{
	...
	$content = reload_viz_d3_barchart_inverted($sid,$value,$options,$setup);
...
<script type="text/javascript">
    ...
    function grouped_d3_barchart_inverted'.$sid.'(labels, data, redraw)
    {
        ...
        var w = '.$dashboard_options['width'].',
            ...
            x = d3.scale.linear().domain([data_max,'.$dashboard_options['minValue'].']).range([h-220, 0]),
	...
	var negative_height = -1* ('.$dashboard_options['height'].' - 220);
        ...
        if (redraw== true)
        {
            ...
            svg.selectAll("rect")
                ...
                .duration(500)
                //.attr("height", x)
                //.attr("height",function(d) { return (280-x(d));})
                .attr("height",function(d) { return (-1 * negative_height-x(d));})
	...
	//.attr("value",function(d){return (h-x(d));})
            .attr("transform", function(d, i) {console.log("y 0 is --> "+y0(i)+" x of d is --> "+(500-x(d))); return "translate(" + y0(i) + ", " + negative_height + ")"; })
            ...
            //.attr("height",function(d) {return  280-x(d);})
            .attr("height",function(d) {return  -1 * negative_height-x(d);})
            .attr("y", function(d) {console.log("The individual plot is "+x(d)); return h; });
...
// Changed from 150 to 185 and 142 to 177	
	...
	// Changed attribute of y from 30 to 40
        vis.append("svg:text")
            .attr("class","xTitle1")
            .attr("x",(w-160)/2)
            .attr("y",h+40)
    ...
    function stacked_d3_barchart_inverted'.$sid.'(labels,data,redraw)
    {
        var m = [0, 100, 140, 150]; // margins
        ...
        var x = d3.scale.ordinal().rangeBands([0, w-180], .2),
            y = d3.scale.linear().range([0, h-160]);
...
//                .attr("fill","#'.$dashboard_options['vAxisColor'].'")
    ...
    function draw_d3_barchart_inverted'.$sid.'(legendLabels,data,redraw)
    {
        if ('.$dashboard_options['isStacked'].') 
            stacked_d3_barchart_inverted'.$sid.'(legendLabels,data,redraw);
        else 
            grouped_d3_barchart_inverted'.$sid.'(legendLabels,data,redraw);
    }

    '.$content.'
    draw_d3_barchart_inverted'.$sid.'(legendLabels'.$sid.',data'.$sid.');
</script>';	
	...
	$str.= '<script language="JavaScript" type="text/javascript">
				...
				function reload_update'.$sid.'(response)
				{
					...
					draw_d3_barchart_inverted'.$sid.'(legendLabels'.$sid.',data'.$sid.',redraw);
...
function reload_viz_d3_barchart_inverted($sid, $value, $options, $setup)
{
	global $db;
