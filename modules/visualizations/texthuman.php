<?
function getsetup_viz_texthuman()
{
	$options['_CREDITS']				= 'Main Developters: Rahul Salla.';
...
function place_viz_texthuman($sid, $value, $options, $setup)
{	$str='';
	$str.= '<div id="cover'.$sid.'">';
	$str.= reload_viz_texthuman($sid, $value, $options, $setup);
	$str.= '</div>';
...
function reload_viz_texthuman($sid, $value, $options, $setup)
#function reload_viz_texthuman($sid, $value, $options)
{
	global $db;
...
#	echo "test";
	...
	if(isset($options['tablename']) and ($options['tablename']!=''))
	{
		...
		$formatted_number = bd_nice_number(floatval($result[0]['value']));
	...
	{
		$content .= bd_nice_number(floatval($value));
	}
