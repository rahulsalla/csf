<?
function getsetup_viz_d3_barchart_test()
{
...
function place_viz_d3_barchart_test($sid, $value, $options, $setup)
{
	...
	$content = reload_viz_d3_barchart_test($sid,$value,$options,$setup);
...
<script type="text/javascript">
    ...
    function grouped_d3_barchart_test'.$sid.'(labels, data, redraw)
    {
            ...
            if ("'.$dashboard_options['vAxisStyle'].'" == "rule"){
               ...
               //     .data(x.ticks(12))
                   ...
                   //.attr("transform", function(d) { return "translate(0," + (h-x(d)) + ")"; });
                    .attr("transform", function(d) { return "rotate(90)translate(150," + (20+x(d)) + ")"; });
...
//Change needed here
		...
		//Translate over the Y axis, changed from -800 to -685
            //.attr("transform", function(d, i) { return "rotate(90)translate(120,-10)"; })
            .attr("transform", function(d, i) { return "rotate(90)translate(150,-80)"; })
    ...
    function stacked_d3_barchart_test'.$sid.'(labels,data,redraw)
    {
        ...
        if (redraw== true)
        {
            ...
            if ("'.$dashboard_options['vAxisStyle'].'" == "rule"){
                ...
                var rule = svg.select("g").selectAll("g.rule")
                 ...
                 rule.append("svg:line")
               //      .attr("x2", w -180)
                     .attr("x2", '.$dashboard_options['yRange'].')
                     .style("stroke", function(d) { return d ? "#ccc" : "#'.$dashboard_options['hAxisLineColor'].'"; })
                     .style("stroke-opacity", function(d) { return d ? .7 : null; });
...
//                .attr("fill","#'.$dashboard_options['vAxisColor'].'")
    ...
    function draw_d3_barchart_test'.$sid.'(legendLabels,data,redraw)
    {
        if ('.$dashboard_options['isStacked'].') 
            stacked_d3_barchart_test'.$sid.'(legendLabels,data,redraw);
        else 
            grouped_d3_barchart_test'.$sid.'(legendLabels,data,redraw);
    }

    '.$content.'
    draw_d3_barchart_test'.$sid.'(legendLabels'.$sid.',data'.$sid.');
</script>';	
	...
	$str.= '<script language="JavaScript" type="text/javascript">
				...
				function reload_update'.$sid.'(response)
				{
					...
					draw_d3_barchart_test'.$sid.'(legendLabels'.$sid.',data'.$sid.',redraw);
...
function reload_viz_d3_barchart_test($sid, $value, $options, $setup)
{
	global $db;
