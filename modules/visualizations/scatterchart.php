<?
function getsetup_viz_scatterchart()
{
	$options['_CREDITS']				= 'Main Developters: Robert Pahle, Jaycen Horton, Erzhena Soktoeva.';
	$options['_MODULEDESCRIPTION']		= 'Each row in the table represents a set of data points with the same x-axis value.
	...
	<p>Columns:</p>
		...
		For more detailed information, please, refer to <a href="https://developers.google.com/chart/interactive/docs/gallery/scatterchart">Google Charts ScatterChart</a>';
	...
	$options[140]['detail']				= 'This is the color of the legend font in the chart';
...
function place_viz_scatterchart($sid, $value, $options, $setup)
{
	...
	if(isset($options['tablename']))
	{
		...
		<script type="text/javascript">
			var scatter_chart'.$sid.';
			var data'.$sid.';
			google.load(\'visualization\', \'1\', {\'packages\':[\'corechart\']});
			google.setOnLoadCallback(drawChart'.$sid.');
			function drawChart'.$sid.'()
			{';
				$content .= reload_viz_scatterchart($sid, $value, $options, $setup);
				$content .= '
				scatter_chart'.$sid.' = new google.visualization.ScatterChart(document.getElementById(\'velement'.$sid.'\')).draw(data'.$sid.', {backgroundColor:\''.($dashboard_options['backgroundColor']).'\',
...
function reload_viz_scatterchart($sid, $value, $options, $setup)
{
