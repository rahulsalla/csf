<?
function getsetup_viz_d3_barchart()
{
    $options['_CREDITS']                = 'Main Developer: Jie Fang, Rahul Salla';
...
function place_viz_d3_barchart($sid, $value, $options, $setup)
{
	...
	$content = reload_viz_d3_barchart($sid,$value,$options,$setup);
...
<script type="text/javascript">
    ...
    function grouped_d3_barchart'.$sid.'(labels, data, redraw)
    {
        ...
        if("'.$dashboard_options['maxValue'].'" == "auto")
        {
        ...
        if("'.$dashboard_options['minValue'].'" == "auto")
        {
            var data_min = d3.min(data, function(d){return d3.min(d);});
        }else
        {
            var data_min = '.$dashboard_options['minValue'].';
        }
        ...
        var w = '.$dashboard_options['width'].',
            ...
            //x = d3.scale.linear().domain([data_max, data_min]).range([h-220, 0]),
...
// Changed from 150 to 185 and 142 to 177	
    ...
    function stacked_d3_barchart'.$sid.'(labels,data,redraw)
    {
...
//                .attr("fill","#'.$dashboard_options['vAxisColor'].'")
    ...
    function draw_d3_barchart'.$sid.'(legendLabels,data,redraw)
    {
        if ('.$dashboard_options['isStacked'].') 
            stacked_d3_barchart'.$sid.'(legendLabels,data,redraw);
        else 
            grouped_d3_barchart'.$sid.'(legendLabels,data,redraw);
    }

    '.$content.'
    draw_d3_barchart'.$sid.'(legendLabels'.$sid.',data'.$sid.');
</script>';	
	...
	$str.= '<script language="JavaScript" type="text/javascript">
				...
				function reload_update'.$sid.'(response)
				{
					...
					draw_d3_barchart'.$sid.'(legendLabels'.$sid.',data'.$sid.',redraw);
...
function reload_viz_d3_barchart($sid, $value, $options, $setup)
{
	global $db;
