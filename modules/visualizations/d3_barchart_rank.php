<?
function getsetup_viz_d3_barchart_rank()
{
...
function place_viz_d3_barchart_rank($sid, $value, $options, $setup)
{
	...
	$content = reload_viz_d3_barchart_rank($sid,$value,$options,$setup);
...
<script type="text/javascript">
    ...
    function grouped_d3_barchart_rank'.$sid.'(labels, data, redraw)
    {
        ...
        for (i=0; i<n; i++) 
            label[i] = odata[i][0];
       // Dashboard width changes changed variable w, changed y0, rangebands 160 to 235
	// Changed attribute x2 to w-235 from w-160 
        var w = '.$dashboard_options['width'].'-50,
            h = '.$dashboard_options['height'].',
            x = d3.scale.linear().domain([data_max,0]).range([h-220, 0]),
            y0 = d3.scale.ordinal().domain(d3.range(n)).rangeBands([0, w-235], .2),
        ...
        var rect = g.selectAll("rect")
            ...
            .on("mouseover",function(){
                d3.select(this)
                    .append("svg:title")
                    .text(function(d){return label[d3.select(this.parentNode).attr("id")]+"\n"+legendLabel[d3.select(this.parentNode.parentNode).attr("legendid")]+": "+(51-d);})       
                    })
...
// Change the legendX and legendTextX to adjust the x coordinate position of legend.	changed from 185 to 235 
// Changed from 177 to 227
	    if (legendlocation == "right") {
            ...
            vis.selectAll("circle")
                ...
                .on("mouseover",synchronizedMouseOver)
                .on("mouseout",synchronizedMouseOut);
            // add legend text// Changed the font size of the legend here from 12 to 20, should include it as an option 
            vis.selectAll("a")
...
// Changed attribute x2 to w-235 from w-160 
        ...
        //Add title // For the long title changed the y attribute to 145
        vis.append("svg:text")
            .attr("x",(w-160)/2)
            .attr("y",145)
            .attr("text-anchor","middle")
    ...
    function stacked_d3_barchart_rank'.$sid.'(labels,data,redraw)
    {
...
//                .attr("fill","#'.$dashboard_options['vAxisColor'].'")
    ...
    function draw_d3_barchart_rank'.$sid.'(legendLabels,data,redraw)
    {
        if ('.$dashboard_options['isStacked'].') 
            stacked_d3_barchart_rank'.$sid.'(legendLabels,data,redraw);
        else 
            grouped_d3_barchart_rank'.$sid.'(legendLabels,data,redraw);
    }

    '.$content.'
    draw_d3_barchart_rank'.$sid.'(legendLabels'.$sid.',data'.$sid.');
</script>';	
	...
	$str.= '<script language="JavaScript" type="text/javascript">
				...
				function reload_update'.$sid.'(response)
				{
					...
					draw_d3_barchart_rank'.$sid.'(legendLabels'.$sid.',data'.$sid.',redraw);
...
function reload_viz_d3_barchart_rank($sid, $value, $options, $setup)
{
	global $db;
