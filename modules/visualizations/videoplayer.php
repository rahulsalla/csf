<?
function getsetup_viz_videoplayer()
{
	$options['_CREDITS'] = 'Main Developters: Rahul Salla, Robert Pahle.';
	$options['_MODULEDESCRIPTION']		= 'Allows for playing and controlling a videoplayer video';
	...
	$options[30]['description']			= 'Width of the videoplayer window';
	...
	$options[40]['description']			= 'Height of the videoplayer window';
...
function place_viz_videoplayer($sid, $value, $options, $setup)
{
...
#	echo '<div id="cover'.$sid.'">';
	...
	echo '</div>';
	echo reload_viz_videoplayer($sid, $value, $options, $setup);
	echo '</div>';
...
function reload_viz_videoplayer($sid, $value, $options, $setup)
{
	$dashboard_options = $options['dashboard_options'];	
	...
	$content = '<div id="velement'.$sid.'" style="position:absolute;z-index:1; top:'.$dashboard_options['y'].'; left:'.$dashboard_options['x'].'; width:'.$dashboard_options['width'].'; height:'.$dashboard_options['height'].';">';
	
	$content = '<div id="video'.$sid.'" style="position:absolute;z-index:100; top:'.$dashboard_options['y'].'; left:'.$dashboard_options['x'].'; width:'.$dashboard_options['width'].'; height:'.$dashboard_options['height'].';"> <video width="'.$dashboard_options['width'].'" height="'.$dashboard_options['height'].'" controls> ';
    $content .= '<source src="files/video/'.$value.'" type="video/mp4">Your browser does not support the video tag.</video>';
	$content .= '</div></div>';
	return($content);
}
